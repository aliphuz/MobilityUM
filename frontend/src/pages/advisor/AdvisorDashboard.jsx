...
import {
  Box, Typography, Grid, Paper, Card, CardContent, Tabs, Tab,
  TextField, Button, Avatar, Chip, Alert, CircularProgress,
  Collapse, Stack, Divider, Link, Stepper, Step, StepLabel, Dialog, DialogTitle,
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                        <Stack direction="row" spacing={1} alignItems="center">
                          <Avatar sx={{ width: 24, height: 24, fontSize: 10, bgcolor: 'primary.light', color: 'primary.contrastText' }}>{initials}</Avatar>
...
function StudentCard({ student, onAccept, acceptMutation }) {
  ...
  const initials = student.studentName ? student.studentName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : ''
      ...
      <Collapse in={open} sx={{ mt: 3 }}>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <Typography variant="subtitle2" fontWeight={700} mb={2}>Supervised Student Profile Details</Typography>
            <Stack spacing={2}>
          ...
          <Grid item xs={12} md={6}>
            <Typography variant="subtitle2" fontWeight={700} mb={2}>Document & OCR Details</Typography>
            <Stack spacing={2.5}>
              {student.passportUploaded ? (
                <Paper variant="outlined" sx={{ p: 2, bgcolor: 'background.paper' }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                    <Typography variant="body2" fontWeight={700}>PASSPORT</Typography>
              ...
              {student.visaUploaded ? (
                <Paper variant="outlined" sx={{ p: 2, bgcolor: 'background.paper' }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                    <Typography variant="body2" fontWeight={700}>VISA</Typography>
              ...
              {student.transcriptUploaded ? (
                <Paper variant="outlined" sx={{ p: 2, bgcolor: 'background.paper' }}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1}>
                    <Typography variant="body2" fontWeight={700}>ACADEMIC TRANSCRIPT</Typography>
        ...
        <Divider sx={{ my: 3 }} />
        <Typography variant="subtitle2" fontWeight={700} mb={1.5}>Application History</Typography>
...
function AdvisorDashboardContent() {
  ...
  const { data: requestsData = 0, isLoading: requestsLoading } = useQuery({
    queryKey: ['advisorrequests'],
    queryFn: () => api.get('/advisor/requests').then(r => r.data),
  })
  const reqCount = typeof requestsData === 'number' ? requestsData : (requestsData?.count || 0)

  // 3. Fetch Pending Applications
  const { data: pendingApplications = [], isLoading: appsLoading } = useQuery({
    queryKey: ['advisorpendingapplications'],
    queryFn: () => api.get('/advisor/applications/pending').then(r => r.data),
  })

  // 4. Add Student Supervision
  const addStudentMutation = useMutation({
    mutationFn: (email) => api.post('/advisor/students/add', { studentEmail: email }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['advisorstudents'] })
  ...
  const reviewMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['advisorpendingapplications'] })
  ...
  return (
    <Box sx={{ px: 3, py: 4 }}>
      <PageHeader
        ...
        subtitle="Manage supervised students and verify application files"
      />
      ...
      {activeTab === 0 && (
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          {/* Supervise a New Student - Card + CardContent */}
          <Card>
            <CardContent sx={{ p: 3 }}>
          ...
          <Box>
            ...
            {studentsLoading ? (
              ...
              <Alert severity="info" sx={{ borderRadius: 2 }}>
                You are not supervising any students yet.
              </Alert>
            ) : (
              <Box>
          ...
          {appsLoading ? (
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2.5 }}>
              {pendingApplications.map((app) => {
                const initials = app.studentName ? app.studentName.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase() : ''
                return (
                  <Paper key={app.id} variant="outlined" sx={{ p: 2.5, borderRadius: 2, mb: 2 }}>
                    <Box sx={{ display: 'flex', gap: 2, alignItems: 'flex-start', mb: 2, flexWrap: 'wrap' }}>
                      <Avatar sx={{ bgcolor: 'warning.light', color: 'warning.contrastText', fontWeight: 700 }}>{initials}</Avatar>
                      <Box sx={{ flex: 1 }}>
                        <Typography fontWeight={700} fontSize={15}>{app.studentName}</Typography>
                    ...
                    <Collapse in={Boolean(expandedApps[app.id])} sx={{ mt: 2 }}>
                      <Divider sx={{ my: 2 }} />
                      <Typography variant="subtitle2" fontWeight={700} mb={2}>Student OCR Profile Details</Typography>
                      
                      <Grid container spacing={3}>
                        {app.studentPassportOcrData && (
                          <Grid item xs={12} md={6}>
                            <Typography variant="body2" fontWeight={700} color="primary" mb={1}>PASSPORT OCR INFO</Typography>
                            <PassportOcrTable ocrData={app.studentPassportOcrData} />
                          </Grid>
                        )}
                        {app.studentTranscriptOcrData && (
                          <Grid item xs={12} md={6}>
                            <Typography variant="body2" fontWeight={700} color="primary" mb={1}>TRANSCRIPT OCR INFO</Typography>
                    ...
                    <Typography variant="subtitle2" fontWeight={700} mb={1.5}>Approval History Logs</Typography>
                    {app.approvalLogs && app.approvalLogs.length > 0 ? (
                      <Box sx={{ display: 'grid', gap: 1 }}>
                        {app.approvalLogs.map((log) => {
                          ...
                          return (
                            ...
                            >
                              <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1 }}>
                                <Stack direction="row" spacing={1} alignItems="center">
                                  <Avatar sx={{ width: 24, height: 24, fontSize: 10, bgcolor: 'primary.light', color: 'primary.contrastText' }}>{logInitials}</Avatar>
                                  <Typography variant="subtitle2" sx={{ fontSize: 12, fontWeight: 700 }}>{log.actorName}</Typography>
                                  <Chip label={log.actorRole} size="small" variant="outlined" sx={{ height: 18, fontSize: 9, fontWeight: 600 }} />
                                </Stack>
                                <Typography variant="caption" color="text.secondary" sx={{ fontSize: 10 }}>
...
export default function AdvisorDashboard() {
  return (
    <Layout>
      <Routes>
        <Route path="/" element={<AdvisorDashboardContent />} />
      </Routes>
    </Layout>
  )
}
